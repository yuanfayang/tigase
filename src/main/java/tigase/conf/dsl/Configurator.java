...
//						public Object call(Object[] args) {
//							System.out.println("executing closure for " + key + " with params = " + Arrays.toString(args));
...
//					System.out.println("executed " + beanName + " with args " + Arrays.toString(args));
//					if (args == null || args.length == 0)
//						return null;
//					
//					Closure c1 = (Closure) args[0];
...
//					return c1.call();
	...
	private class BeansBinder extends Binding {
		...
		public BeansBinder(Kernel kernel, Binding binding) {	
			...
			this.kernel = kernel;
			this.kernel.registerBean(AbstractBeanConfigurator.DEFAULT_CONFIGURATOR_NAME).asClass(ConfigurationProvider.class).exportable().exec();
		}
		...
		public Object invokeMethod(String name, Object args_) {
			try {
				...
				Object configurator = kernel.getInstance(AbstractBeanConfigurator.DEFAULT_CONFIGURATOR_NAME);
			...
			Registrar reg = new Registrar() {
				...
				public void register(Kernel kernel) {
					try {
						...
						Object configurator = kernel.getInstance(AbstractBeanConfigurator.DEFAULT_CONFIGURATOR_NAME);
						if (configurator != null && configurator instanceof ConfigurationProvider) {
							((ConfigurationProvider) configurator).setBeanConfiguration(name, KernelBinder.this, props, closure);
						}
					} catch (ClassNotFoundException ex) {
						Logger.getLogger(Configurator.class.getName()).log(Level.SEVERE, null, ex);
					}			
	...
	public static class ConfigurationProvider extends AbstractBeanConfigurator {
		...
		public void configure(BeanConfig beanConfig, Object bean) throws KernelException {
			...
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "configuring instance of bean {0} with {1}", new Object[]{beanConfig.getBeanName(), configProps});
			}
			super.configure(beanConfig, bean);
		}
